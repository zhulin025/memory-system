 ...
 * 检查是否应该触发 Dream
 */
export async function shouldTriggerDream(
  projectId?: string,
  options: {
    minHours?: number
    minSessions?: number
 ...
 * 获取距离上次 Dream 的小时数
 */
async function getHoursSinceLastDream(projectId?: string): Promise<number> {
 ...
 * 简化实现：统计新增的记忆文件数量
 */
async function countSessionsSinceLastDream(projectId?: string): Promise<number> {
  const lockPath = getDreamLockPath(projectId)
 ...
 * 尝试获取 Dream 锁
 */
export async function tryAcquireDreamLock(projectId?: string): Promise<boolean> {
  const lockPath = getDreamLockPath(projectId)
...
export async function releaseDreamLock(
  ...
  if (priorMtime === undefined) return
 ...
 * 执行 Dream 整理
 */
export async function runDream(projectId?: string): Promise<DreamResult> {
...
export function buildDreamPrompt(
  memoryRoot: string,
  maxIndexLines: number = 200,
  maxIndexBytes: number = 25000
): string {
  return `
# Dream: 记忆整理
