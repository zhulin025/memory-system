 ...
 * 更新配置
 */
export function configure(newConfig: Partial<MemorySystemConfig>): void {
 ...
 * 获取配置
 */
export function getConfig(): MemorySystemConfig {
  return { ...config }
}
 ...
 * @param projectId 项目 ID（可选，默认自动检测）
 */
export async function dream(projectId?: string): Promise<DreamResult> {
 ...
 * 检查 Dream 状态
 */
export async function checkDreamStatus(projectId?: string): Promise<{
 ...
 * 移除记忆索引条目
 */
export async function removeMemory(file: string, projectId?: string) {
