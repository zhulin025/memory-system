 ...
 * 从消息提取候选记忆
 */
function extractCandidateFromMessage(_msg, content) {
    // 推断记忆类型
 ...
 * 处理候选记忆
 */
async function processCandidate(candidate, existingMemories, projectId) {
    // 检查是否已存在（避免重复）
 ...
 * 查找相似记忆
 */
function findSimilarMemory(candidate, existing) {
    return existing.find(mem => {
        // 类型相同且标题相似
        if (mem.frontmatter.type !== candidate.type)
            return false;
        const titleSimilar = levenshteinDistance(mem.frontmatter.title.toLowerCase(), candidate.title.toLowerCase()) < 10;
        return titleSimilar;
    });
}
/**
 * 保存新记忆
 */
async function saveMemory(candidate, projectId) {
 ...
 * 更新现有记忆
 */
async function updateMemory(existing, candidate, projectId) {
    const frontmatter = {
 ...
 * 加载现有记忆
 */
async function loadExistingMemories(projectId) {
    const memories = [];
 ...
 * 格式化记忆文件
 */
function formatMemoryFile(frontmatter, content) {
    return `---
 ...
 * 生成文件名
 */
function generateFileName(candidate) {
