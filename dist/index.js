 ...
 * 读取索引文件
 */
export async function readIndex(projectId) {
 ...
 * 解析索引内容
 */
export function parseIndexContent(content) {
    const entries = [];
 ...
 * 从文件名推断记忆类型
 */
function inferMemoryTypeFromFile(file) {
 ...
 * 更新索引
 */
export async function updateIndex(entries, projectId) {
 ...
 * 格式化索引内容
 */
export function formatIndexContent(entries) {
 ...
 * 添加条目到索引
 */
export async function addIndexEntry(entry, projectId) {
 ...
 * 从索引移除条目
 */
export async function removeIndexEntry(file, projectId) {
    const entries = await readIndex(projectId);
 ...
 * 验证索引是否符合限制
 */
export function validateIndex(content) {
    const lines = content.split('\n');
 ...
 * 修剪索引（如果超出限制）
 */
export function truncateIndex(content) {
    const lines = content.split('\n');
    // 先按行数修剪
 ...
 * 格式化索引验证警告
 */
export function formatIndexWarning(lineCount, byteCount, wasLineTruncated, wasByteTruncated) {
