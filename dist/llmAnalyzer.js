...
请分析并返回 JSON：`;
     ...
     * 调用 Anthropic API
     */
    async callAnthropic(prompt, apiKey, model, baseUrl) {
     ...
     * 调用 OpenAI API
     */
    async callOpenAI(prompt, apiKey, model, baseUrl) {
     ...
     */
    async callQwen(prompt, apiKey, model, baseUrl) {
     ...
     * 评估记忆重要性（独立调用）
     */
    async evaluateImportance(content, context) {
...
返回 JSON：
{
  ...
  "reason": "评估理由"
     ...
     * 检查重复
     */
    async checkDuplicate(newMemory, existingMemories) {
